...
void betaOptimize(Axis& a){
    ...
    for(int iter=0; iter<a.itermax; iter++){
        double mid = 0.5*(low+high);
        a.beta1 = mid;
        generateAxis(a);
        double errorMid = (a.d[a.N1-1]- a.d[a.N1-2])/a.d[a.N1-2];
        if(abs(errorMid)<a.tolerance) break;
        if(a.error[0]*errorMid<0) high = mid;
        else { low = mid; a.error[0]=errorMid;}
        a.beta1_hist.push_back(a.beta1);
        a.error1_hist.push_back(errorMid);
    }
    ...
    for(int iter=0; iter<a.itermax; iter++){
        ...
        a.beta2 = mid;
        generateAxis(a);
        double errorMid = (a.d[a.N1+a.Ns-2]- a.d[a.N1+a.Ns-3])/a.d[a.N1+a.Ns-3];
        if(abs(errorMid)<a.tolerance) break;
        if(a.error[1]*errorMid<0) high = mid;
        else { low = mid; a.error[1]=errorMid;}
        a.beta2_hist.push_back(a.beta2);
        a.error2_hist.push_back(errorMid);
    }
