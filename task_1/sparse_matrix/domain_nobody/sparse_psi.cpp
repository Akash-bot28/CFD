...
*/
void BCPsi(const MeshParameter& p,const Mesh& m,Coefficient& c,Field& f){
    ...
    for(n=0; n<s.itermax; n++){
        ...
        for(int j=1; j<p.Ny-1; j++){
            for(int i=1; i<p.Nx-1; i++){
                f.psi[i][j] = (c.bP[i][j]- c.aS[i][j]*f.psi[i][j-1]- c.aW[i][j]*f.psi[i-1][j]- c.aE[i][j]*f.psi[i+1][j]- c.aN[i][j]*f.psi[i][j+1])/c.aP[i][j];
            }
        }
        ...
        for(int j=1; j<p.Ny-1; j++){
            for(int i=1; i<p.Nx-1; i++){
                double r = c.bP[i][j] - c.aS[i][j]*f.psi[i][j-1] - c.aW[i][j]*f.psi[i-1][j] - c.aP[i][j]*f.psi[i][j] - c.aE[i][j]*f.psi[i+1][j] - c.aN[i][j]*f.psi[i][j+1];
                sumofsquares += r*r;
            }
        }
...
void Jacobi(SolverParameter& s,const MeshParameter& p,const Mesh& m,const Coefficient& c,Field& f){
    ...
    cout << "\nJacobi Solver iterations = "<< n << endl;
