...
data_BiCG= np.loadtxt("psi_BiCGSTAB_converge.dat")
iter_BiCG=data_BiCG[:,0]
error_BiCG=data_BiCG[:,1]
iter_BiCG_final = int(iter_BiCG[-1])
plt.loglog(iter_BiCG,error_BiCG,'-',linewidth=2,label=f'BiCG : {iter_BiCG_final}')

plt.legend()
plt.title("PSI_solver convergence")
plt.xlabel("log(iteration)")
plt.ylabel("log(rms error)")
plt.grid(True)

plt.savefig("PSI_solver_convergence.png", dpi=800, bbox_inches='tight')
