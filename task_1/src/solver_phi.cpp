...
void BoundaryValues(const Domain& d, const MeshParameter& p,const Mesh& m,Field& f){
    ...
    for(int j=0; j<p.Ny; j++) f.phi[0][j] = U*d.XL + phi0;      // Left boundary (inlet)
    for(int j=0; j<p.Ny; j++) f.phi[p.Nx-1][j] =U*d.XR + phi0;  // Right boundary (outlet)
    for(int i=0; i<p.Nx; i++) f.phi[i][0] = U*m.x[i][0] + phi0; // Bottom boundary
    for(int i=0; i<p.Nx; i++) f.phi[i][p.Ny-1] = U*m.x[i][p.Ny-1] + phi0; // Top boundary
}
...
void Jacobi(SolverParameter& s,const Body& b,const MeshParameter& p,const Coefficient& c,Field& f){
    ...
    cout << "\nPHI Jacobi Solver iterations = "<< n << endl;
...
void BiCGSTAB(SolverParameter& s, const Body& b, const MeshParameter& p, const Mesh& m, const Coefficient& c, Field& f) {
    cout << "\nSolving Phi using BiCGSTAB..." << endl;
    s.error.clear();

    // Initialization
    // Compute initial residual: r0 = bP - A*phi
    vector<vector<double>> Ax = matVecProduct(p, c, f.phi);
    ...
    for(n=1; n<s.itermax; n++){
        ...
        for (int j = 1; j < p.Ny - 1; j++) {
            for (int i = 1; i < p.Nx - 1; i++) {
                f.phi[i][j] += alpha * p_vec[i][j] + omega * s_vec[i][j];
    ...
    cout << "\nPHI BiCGSTAB Solver iterations = " << n << " | Final RMS = " << rms << endl;
    if (n == s.itermax) cout << "Stopped due to itermax." << endl;
    else cout << "Converged." << endl;
...
int main(){
    ...
    exportSolverData(s,p,m,c,f,"phi_BiCGSTAB_converge.dat");
    exportPhi(p,m,f,"phi_BiCGSTAB_solved.dat");
    return 0;
}